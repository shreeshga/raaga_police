...
static const double FFT_MINFREQ = 45.0; //100.0; //45.0;
static const double FFT_MAXFREQ = 1000.0 ;//5000.0;
...
void Tone::print() const {
	char notes[][3] = {{"SA"},{"ri"},{"RI"},{"ga"},{"GA"},{"ma"},{"MA"},{"PA"},{"da"},{"DA"},{"ni"},{"NI"}};
	
	double note = 33.0 + 12.0 * log(freq / 440.0) / log(2.0);
	if (note >= 0.0 && note < 100.0) note+= 0.5;
	//
	//if (age < Tone::MINAGE) return;
	if (note < 0 ) return;
	std::cout << notes[(int)note%12] << ": Note  "<<std::fixed << std::setprecision(1) << freq << " Hz, age " << age << ", " << db << " dB:";
	for (std::size_t i = 0; i < 4; ++i) std::cout << " " << harmonics[i];
	std::cout << std::endl;
}
...
bool Analyzer::calcFFT() {
	float pcm[FFT_N];
	size_t r = m_bufRead; //shreesh
	if ((BUF_N + m_bufWrite - r) % BUF_N <= FFT_N)
		return false;
	for (size_t i = 0; i < FFT_N; ++i) {
...
void Analyzer::calcTones() {
	...
	for (size_t k = kMax - 1; k >= kMin; --k) {
		...
		for (std::size_t div = 2; div <= Tone::MAXHARM && k / div > 1; ++div) {
			...
			int score = 0;
			for (std::size_t n = 1; n < div && n < 16; ++n) {
		...
		//Mess around these values if it does not recognize some of lower db notes
		if (t.db > 35.0 - 3.0 * count) {
//		if (t.db > 15.0 - 3.0 * count) {
			t.stabledb = t.db;
			tones.push_back(t);
		}
